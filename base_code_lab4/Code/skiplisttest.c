 ...
 */
void print_ele(int ele,void * none){
	printf("%d ",ele);
	none=(void *)none;
}
...
void test_search(int num){
	...
	input = fopen(constructfromfile, "r");
	if (input!=NULL) {
		unsigned int nb_tests = read_uint(input);
		int nb_trouve=0;
		unsigned int max_op=0;
		unsigned int min_op=skiplist_size(d);
		unsigned int moyenne=0;
		unsigned int nb_op_act=0;
		int val_cherchee;
		for (unsigned int i=0;i< nb_tests; ++i) {
			nb_op_act=0;
			val_cherchee=read_int(input);
			if (skiplist_search(d, val_cherchee,&nb_op_act)){
				printf("%d -> true\n",val_cherchee);
				nb_trouve++;
			}
			else printf("%d -> false\n",val_cherchee);
			if(nb_op_act>max_op) max_op=nb_op_act;
			if(nb_op_act<min_op) min_op=nb_op_act;
			moyenne+=nb_op_act;
		}
		moyenne=moyenne/nb_tests;
		printf("Statistics :\n	Size of the list : %d\n",skiplist_size(d));
		printf("Search %d values :\n	Found %d\n	Not found %d\n	Min number of operations : %d\n	Max number of operations : %d\n	Mean number of operations : %d\n",nb_tests,nb_trouve,nb_tests-nb_trouve,min_op,max_op,moyenne);
	} else {
		printf("Unable to open file %s\n", constructfromfile);
		free(constructfromfile);
		exit (1);
